...
export class WebRTCAdaptor {
  ...
  constructor(initialValues) {
     ...
     */
    this.remoteVideo =
      this.remoteVideoElement || document.getElementById(this.remoteVideoId);
  ...
  sendPublishCommand(
    ...
    let jsCmd = {
      ...
      token: token,
      subscriberId:
        typeof subscriberId !== undefined && subscriberId != null
          ? subscriberId
          : "",
      subscriberCode:
        typeof subscriberCode !== undefined && subscriberCode != null
          ? subscriberCode
          : "",
      streamName:
        typeof streamName !== undefined && streamName != null ? streamName : "",
      mainTrack:
        typeof mainTrack !== undefined && mainTrack != null ? mainTrack : "",
      video: videoEnabled,
      audio: audioEnabled,
      metaData:
        typeof metaData !== undefined && metaData != null ? metaData : "",
    };
  ...
  play(
    ...
    let jsCmd = {
      ...
      trackList:
        typeof enableTracks !== undefined && enableTracks != null
          ? enableTracks
          : [],
      subscriberId:
        typeof subscriberId !== undefined && subscriberId != null
          ...
          : "",
      subscriberCode:
        typeof subscriberCode !== undefined && subscriberId != null
          ? subscriberCode
          : "",
      viewerInfo:
        typeof metaData !== undefined && metaData != null ? metaData : "",
  ...
  tryAgain() {
    ...
    for (var index in this.playStreamId) {
      ...
      if (
        ...
        logger.log(
          "It will try to play again for stream: " +
            streamId +
            " because it is not stopped on purpose"
        );
        this.stop(streamId);
        setTimeout(() => {
      ...
      if (!this.playStreamId.includes(streamId)) {
        if (this.mediaManager.localStream != null) {
          this.mediaManager.localStream.getTracks().forEach((track) => {
            let rtpSender = this.remotePeerConnection[streamId].addTrack(
              track,
            ...
            if (track.kind == "video") {
              ...
              rtpSender
                ...
                .then(() => {
                  logger.info(
                    "Degradation Preference is set to " +
                      this.degradationPreference
                  );
                })
                .catch((err) => {
                  logger.warn(
                    "Degradation Preference cannot be set to " +
                      this.degradationPreference
                  );
                });
            }
    ...
    return new Promise((resolve, reject) => {
      this.remotePeerConnection[streamId]
        ...
        .then((stats) => {
          ...
          this.remotePeerConnectionStats[streamId].totalVideoPacketsSent =
            videoPacketsSent;
          this.remotePeerConnectionStats[streamId].totalAudioPacketsSent =
            audioPacketsSent;
          ...
          this.remotePeerConnectionStats[streamId].audioJitterAverageDelay =
            audioJitterAverageDelay;
          this.remotePeerConnectionStats[streamId].availableOutgoingBitrate =
  ...
  sendData(streamId, data) {
    ...
    if (this.remotePeerConnection[streamId] !== undefined) {
      ...
      if (
        ...
        logger.warn(
          "dataChannel.readyState is not open: " + dataChannel.readyState
        );
        return;
      }
      var length = data.length || data.size || data.byteLength;
      ...
      logger.warn(
        "Send data is called for undefined peer connection with stream id: " +
          streamId
      );
    }
  }
  ...
  sendPushNotification(
    ...
    if (!Array.isArray(subscriberIdsToNotify)) {
      logger.error(
        "subscriberIdsToNotify is not an array. Please put the subscriber ids to notify in an array such as [user1], [user1, user2]"
      );
      throw new Error(
        "subscriberIdsToNotify is not an array. Please put the subscriber ids to notify in an array such as [user1], [user1, user2]"
      );
  ...
  switchVideoCameraCapture(streamId, deviceId, onEndedCallback) {
    return this.mediaManager.switchVideoCameraCapture(
      streamId,
      deviceId,
      onEndedCallback
    );
  ...
  updateVideoTrack(stream, streamId, onEndedCallback, stopDesktop) {
    return this.mediaManager.updateVideoTrack(
      stream,
      streamId,
      onEndedCallback,
      stopDesktop
  ...
  updateAudioTrack(stream, streamId, onEndedCallback) {
    return this.mediaManager.updateAudioTrack(
      stream,
      streamId,
      onEndedCallback
    );
